...
function setupWebRTC(iceServerUrl, iceServerUsername, iceServerCredential) {
    ...
    peerConnection.ontrack = function (event) {
        ...
        console.log(`peerConnect.ontrack: kind=${event.track.kind}`)
        if (event.track.kind === 'audio') {
            ...
            audioElement.onloadedmetadata = (md) => {
                console.log(`WebRTC loadedmetadata: video metadata ready: ${JSON.stringify(md)}`);
            }
            audioElement.onwaiting = () => {
                console.log("WebRTC: onwaiting audio ")
            }
            audioElement.onplay = () => {
                console.log("WebRTC: onplay, start playing")
            }
            console.log(`WebRTC: adding audio element ${audioElement} to remoteVideo`)
            console.log(`WebRTC: remoteVideo hidden is ${document.getElementById('remoteVideo').hidden} `)
        ...
        if (event.track.kind === 'video') {
            ...
            console.log(`WebRTC: adding video element ${videoElement} to remoteVideo`)
...
function initMessages() {
    ...
    if (dataSources.length === 0) {
        ...
        console.log(`initMessages: systemPrompt=${systemPrompt}`)
